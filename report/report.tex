...
\title{\textbf{Redução Transitiva de Grafos Direcionados\\baseada em Caminhamento em Profundidade}}

\author{
Yrlan Rangel Teixeira$^1$\\[2mm]
\small $^1$Instituto de Ciências Exatas e Informática, PUC Minas\\
\small Projeto e Análise de Algoritmos, Prof. Silvio Jamil F. Guimarães, 2026/1
}
\date{}
...
O grafo é representado por \textbf{lista de adjacência}: um vetor \texttt{adj} de $|V|$ posições em que \texttt{adj[u]} é um \texttt{std::vector<int>} com os sucessores diretos de $u$. A escolha é justificada por três fatores. Primeiro, o algoritmo é inteiramente baseado em caminhamento: uma DFS sobre lista de adjacência custa $O(|V|+|E|)$, pois visita apenas as arestas que de fato existem, ao passo que sobre matriz de adjacência custaria $\Theta(|V|^2)$ por busca, mesmo em grafos esparsos; o custo total do algoritmo saltaria, assim, de $O(|V|(|V|+|E|))$ para $\Theta(|V|^3)$. Segundo, a memória é $O(|V|+|E|)$ contra $\Theta(|V|^2)$ da matriz, o que permite tratar grafos grandes e esparsos. Terceiro, a única operação que a matriz ofereceria em $O(1)$ (consultar se uma aresta específica existe) não é necessária: o algoritmo nunca pergunta ``existe $(u,v)$?'', apenas enumera sucessores. O uso de \texttt{std::vector} contíguo ainda favorece a localidade de \emph{cache} durante as travessias.
...
O caminho correto para grafos direcionados \emph{com ciclos}, por sua vez, seria condensar as componentes fortemente conexas (Tarjan/Kosaraju), reduzir o DAG de condensação e substituir cada componente por um ciclo hamiltoniano interno mínimo, conforme observado por Aho et al.~\cite{aho1972}.
...
O trabalho foi integralmente desenvolvido por \textbf{Yrlan Rangel Teixeira}: implementação de todos os módulos em C++ (representação do grafo, caminhamentos, algoritmo de redução transitiva, gerador de instâncias e verificador de corretude), execução e análise dos experimentos e redação deste relatório.

\begin{thebibliography}{9}
\bibitem{aho1972}
A.~V. Aho, M.~R. Garey, J.~D. Ullman.
\newblock The transitive reduction of a directed graph.
\newblock \emph{SIAM Journal on Computing}, v.~1, n.~2, p.~131 a 137, 1972.

\bibitem{cormen}
T.~H. Cormen, C.~E. Leiserson, R.~L. Rivest, C.~Stein.
\newblock \emph{Introduction to Algorithms}, 4.~ed.
